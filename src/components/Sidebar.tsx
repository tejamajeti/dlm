...
export const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab, userRole = 'Admin' }) => {
  const menuItems = [
    { id: 'overview', label: 'Executive Dashboard', icon: <LayoutDashboard className="w-5 h-5" />, roles: ['Admin', 'Warehouse Manager', 'Operator'] },
    ...
    { id: 'audit', label: 'Kafka Event Log', icon: <ScrollText className="w-5 h-5" />, roles: ['Admin', 'Operator'] },
    { id: 'public-tracker', label: 'Public Order Tracker', icon: <Search className="w-5 h-5" />, roles: ['Admin', 'Warehouse Manager', 'Driver', 'Customer', 'Operator'] },
  ];
  ...
  return (
    <aside className="w-64 glass-panel border-r border-slate-800/80 flex flex-col h-screen sticky top-0">
      ...
      <nav className="flex-1 p-4 space-y-1.5 overflow-y-auto">
        <div className="px-3 pb-2 text-[10px] font-bold uppercase tracking-wider text-slate-500">Main Menu</div>
        {menuItems
      ...
      <div className="p-4 border-t border-slate-800/80">
        <div className="p-3.5 rounded-xl bg-slate-900/90 border border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold text-slate-300">Kafka Bus Online</span>
          </div>
